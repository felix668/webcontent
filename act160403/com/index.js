...
function init() {
  ...
  //微博测试
	...
	window.WeiboJS.init({
		    'appkey' : '1283935721',
		    'debug': true,
		    'timestamp': 1465194622,
		    'noncestr': '12345',
		    'signature': 'cdae66471194c944fddcdcdbd9e7ff720f375d95',
		    'scope': [
		        'getNetworkType'
		    ]
		}, function(ret){
...
Index.fillPage = function () {
    var app = !param("u");
    if (app) {//通过app取用户信息
        Local.reqaObj(server() + "sum", function (data) {
            //处理初始化数据
...
Index.logined = function (data, app) {
    ...
    if (data != '') {//活跃用户
        //初始化昵称和头像
        Local.reqaObj(serverRoot() + "api/user?tf=1&type=4&u=" + Index.u, function (data) {
...
Index.toShare = function () {
    var shareObj = {
        url: front() + "com/index.html?u=" + Index.u || 0,
        title: ($("#username").text() == "" ? "" : "我的") + "QQ阅读时光机-记录阅读点滴",
        cover: front() + "com/images/sgj.jpg",
        intro: ($("#username").text() == "" ? "QQ阅读" : $("#username").text())
        + "这么牛，他咋不上天呢，速来围观",
    };
...
Index.CheckInstall = function () {
    ...
    Appinstalledcheck.checkDownloadApp(function (isAppInstalled) {
        if (isAppInstalled) {
            $(".awaken a").attr("href", "javascript:Index.toReader();");
        }
    });
};
//弹窗显示
...
Index.toReader = function () {
    if (JsBridge.iOS) {
        window.nativechecker.doGoToURL("http://iyuedu.qq.com/event/act160403/ios/index.html");
    } else {
        window.nativechecker.doGoToURL("http://iyuedu.qq.com/event/act160403/adr/index.html");
    }
