...
var _CONF = {
    ...
    SERVER_VERSION: '5_8',
    SERVER_VERSION_TGW: 'v5_8',
    SERVER_COMM_VERSION: 'common',
    TEST_SERVER: 'http://android.reader.bookcs.3g.qq.com',	//测试环境后台服务地址
    TEST_SERVER_TGW: 'http://bookcs.android.reader.qq.com',	//测试环境后台服务地址
    TEST_LOCAL_HOST: 'http://solomotest4.3g.qq.com/book_res/reader/android',

    FORMAL_LOCAL_HOST: 'http://iyuedu.qq.com/android',
    FORMAL_SERVER: 'http://adr.reader.qq.com',		//正式环境后台服务地址
...
//开发环境
if (window.location.hostname == _CONF.TEST_HOST || window.location.hostname == _CONF.LOCAL_HOST_NAME || param("dotest") == 1) {
...
//获取后台服务地址
function servertgw() {
    return _SERVER_TGW + '/' + _CONF.SERVER_VERSION_TGW;
}
...
function pageDomain() {
    ...
    try {
        ...
        if (pageCname.length > 0 && pageCname.indexOf(".") >= 0) {
            return "http://" + pageCname + '/android/' + _CONF.SERVER_VERSION;
        } else {
            return _CONF.FORMAL_LOCAL_HOST + '/' + _CONF.SERVER_VERSION;
        }
    } catch (e) {
