...
Local.resApis = function () {
    JsBridge.restoreApis({
            'JSContent': ['setStatInfo', 'openExternal', 'openDetail', 'closeDialog', 'openMonthly', 'setWebTitlebarIcon', 'openTopicComment', 'openTopicDiscuss', 'setIsBackToPage'],
...
Local.dodefaultLoading = function (callbackSections) {
    ...
    if (callbackSections && callbackSections.length && callbackSections.length > 0) {
        for (var section in callbackSections) {
            ih(callbackSections[section], '<div class="loadingTop"><span class="loadtc"><span class="loads"></span>加载中...</span></div>');
        }
    }
};
...
Local.getExparamWithoutClient = function (url) {
    ...
    //微信身份识别专用
    if (!nl(param("u"))) {
    	params += '&u=' + param("u"); //微信账号
    }
    if (!nl(param("aid"))) {
    	params += '&aid=' + param("aid"); //赠送限制
    }
    //统计专用
    if (!nl(param("act_f"))) {
    	params += '&act_f=' + param("act_f"); //统计
    }
