...
if (window.pt == "adr") {
    var Local = (function(JsBridge, common) {
        ...
        Local.shareTopic = function(url, img, title, intro) {
            ...
            Local.setIconForShareing(url, img, title, intro);
        };
        //单本书加入书架
        Local.addToShelf = function(book) {
                JSAddToShelf.addById(book.bid);
            }
            //批量添加到本地书架  
        Local.addToShelfBooks = function(books) {
            ...
            for (var i = 0; i < books.length; i++) {
                var book = {
                    ...
                    downloadinfo: books[i].downloadinfo
                }
                booklist.push(book);
            }
            Local.addBooks(json(booklist));
        };
        //去书籍详情页
