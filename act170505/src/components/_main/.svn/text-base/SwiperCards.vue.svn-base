...
<div class="SwiperCards">
	...
	<!-- 	<btn-share></btn-share>
		...
		ref="swiper">
			...
			<ul class="train" 
				...
				<li class="item"
				v-if="carousel"
				:class=" currentOne===6?'active':'' ">
					<img class="card" :src=" img+'/main/cards/6.jpg' "/>
				</li>
	...
	<div class="title">
		{{items[currentOne].title}}
	</div>
	<div class="category">
		{{items[currentOne].category}} | {{items[currentOne].finished?'完结共':'连载至'}}{{items[currentOne].chapters}}章
	</div>
</div>
</template>
...
.SwiperCards {
	...
	/*	.arrow_left {
		...
		.Swiper {
			position: relative;
			width: 1.36rem; height: 1.8rem;
			margin: auto; margin-bottom: 0.14rem;
			perspective: 10rem;
