...
const mutators = {
  ...
  TO_MORE_GIFTS({ state, dispatch, getters }, action) {
    var href;
    if (state.meta.platform === 'adr') {
  ...
  TO_UPGRADE({ state, dispatch, getters }, action) {
  ...
  TO_OPEN_VIP({ state, dispatch, getters }, action) {
  ...
  TRY_SHARE({ state, dispatch, getters }, action) {
