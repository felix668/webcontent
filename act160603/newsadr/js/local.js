...
Local.resApis = function () {
    JsBridge.restoreApis({'JSContent': ['openExternal', 'openDetail', 'openNativeDetail', 'openInternal', 'onTabSelected', 'openDetailMore', 'closeDialog', 'openMonthly', 'setWebTitlebarIcon','openDetailForEdit','getTheme'],
            'JSToast': ['showToast'],
            'JSAddToShelf': ['add', 'addBooks'],
            'JSSendBook': ['sendOnlineBook', 'jumpToQQSplash', 'jumpToQQSplashNoUin'],
            'bookdir': ['dir'],
            'sendvip': ['send', 'setvip'],
            'mclient': ['req', 'saveErrInfo', 'switchTab'],
            'JSComment': ['showRateDialog'],
            'JSbookshelf': ['bookAction', 'gotoshelf', 'updateFollowState','setBookShelfShowMode','getBookShelfShowMode'],
            'JSGoToWeb': ['gotoWeb'],
            'JSRoot':['switchTab'],
            'readerlogin': ['login'],
            ...
            'JSPublicAccount': ['isAlreadyFollowPublicAccount', 'followPublicAccount'],
            'JSAdv': ['showadv', 'closeadv', 'getUserInfo', 'closerefresh'],
            'readonline': ['readbook', 'readBookWithGift', 'startOnlineRead'],
            'JSSns': ['shareBook', 'joinQQGroup', 'doMoreState', 'getUserPref', 'sharePage', 'getUserLikeChannel', 'getUserInfo'],
            'JSApp': ['installqqreadermain','openqqreadermain','getreadermainstate','getVersionCode'],
            'JSBookDetailForQQ' : ['pageAction', 'closeNativeButton'],
            'JSPopupListForQQ' :['openList'],
            'JSTopRightButton' :['showRightButton','doAction'],
            'JSSearch' :['doSearch'],
            'JSReload' :['retry', 'goBack'],
            'JSLocalStorage' : ['put','get','remove'],//最好可以放在SD卡存储，保证一般卸载无法删除
    		'JsAdEvent' : ['setStart', 'setEnd']
            },
        '5.0.1');
};

// 首先尝试打开Native详情页，失败则尝试打开html详情页
Local.openBookDetail = function (bid, isLegacy) {
	try {
		if (isLegacy) {
			JSContent.openDetail("bookDetails.html?id=" + bid);
		} else {
			JSContent.openNativeDetail(JSON.stringify({bookId:bid}));
		}
	} catch (e) {
		console.log(e);
	}
...
// 打开NativeDetail
Local.openNativeDetail = function (json) {
	try {
		JSContent.openNativeDetail(json);
	} catch (e) {
		console.log("JSContent.openNativeDetail error", e);
	}
};

//打开NativeDetail
Local.openNativeDetailById = function (bid, origin, feedParam) {
	var obj = {bookId: bid};
	obj.stat_params = {};
	
	if (!nl(origin)) {
		obj.stat_params.origin = origin;
	} else if (!nl(param("origin"))) {
		obj.stat_params.origin = param("origin");
    }
	
	var feed = null;
	if (!nl(feedParam)) {
		feed = feedParam;
	} else if (!nl(param("feedParam"))) {
		feed = param("feedParam");
    }
	if (feed) {
		try {
			feed = JSON.parse(decodeURIComponent(feed));
			for (var prop in feed) {
				if (feed.hasOwnProperty(prop)) {
					obj.stat_params[prop] = feed[prop];
				}
			}
		} catch (e) {
			// ignore
 	...
 	Local.openNativeDetail(JSON.stringify(obj));
};
