			...
			<div class="card">
				...
				<div class="content--">
					<div class="panel-heading">
						<p class="text-left">秒杀第{{i+1}}场</p>
						<p class="text-right"
						v-if="i<=stageNo">已开抢</p>
						<p class="text-right"
						v-if="i>stageNo"
						:style=" i===stageNo+1&&timeNext<900?'color:#d11391':'color:#333333' ">{{i===0?'11':(i===1?'17':'20')}}:00<span v-if="i===stageNo+1&&timeNext<900">即将</span>开抢</p>
					</div>
					<div class="panel-body">
						...
						v-for="n in 2">
							<div class="cover">
								...
								<img class="coverImg" :src=" 'img/covers/'+books[i*2+n-1].bid+'.jpg' "
								@click="TO_BOOK(books[i*2+n-1].bid)"/>
							</div>
							<p class="title">{{books[i*2+n-1].title}}</p>
							<div class="btnBuy">
								<!-- 秒杀成功 -->
								<img class="buy_killed"
								:src=" 'img/buy_killed.png' "
								v-show=" books[i*2+n-1].killed "/>
								<img class="buy_disabled" 
								:src=" 'img/buy_disabled.png' "
								v-show=" !books[i*2+n-1].killed && (i>stageNo)"/>
								<img class="buy"
								:src=" 'img/buy.png' "
								v-show="!books[i*2+n-1].killed&&books[i*2+n-1].left>0&&i<=stageNo"
								@click="KILL(books[i*2+n-1].bid)"/>
								<img class="buy_out" 
								:src=" 'img/buy_out.png' "
								v-show=" !books[i*2+n-1].killed && (books[i*2+n-1].left<=0)"/>
							</div>
							<div class="desc">
								<!-- 秒杀未开始 -->
								<p v-if="i>stageNo">限100册</p>
								<p v-if="i<=stageNo && !books[i*2+n-1].killed && books[i*2+n-1].left===100">限{{books[i*2+n-1].left}}册</p>
								<p v-if="i<=stageNo && !books[i*2+n-1].killed && books[i*2+n-1].left<100 && books[i*2+n-1].left>0">剩余{{books[i*2+n-1].left}}册</p>
								<p v-if="books[i*2+n-1].killed">已加入书架</p>
							</div>
						</div>
					</div>
				</div>
