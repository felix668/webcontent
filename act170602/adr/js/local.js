...
Local.resApis = function () {
    JsBridge.restoreApis({
            'JSContent': ['openExternal', 'openDetail', 'setIsBackToPage','setWebTitlebarIcon'],
...
Local.setIconForShareing = function(url, img_url, title, intro) {
    ...
    Local._share = function() {
        Local.shareTopic(url,img_url,title,intro,1)
    }
    if (window.JSContent) {
 ...
 * 书包页用，但书包页已取消，作废
 */
Local.addToLocalShelfAll = function (bookInfos) {//批量添加到本地书架
