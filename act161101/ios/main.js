	...
	let vm = new Vue({
		...
		components: {
			buyMask: {
				...
				template: '<div class="mask"><div class="tiparea"><h2>确认购买？</h2><p class="bor"><span v-for="n in 3">《{{buyinfo[n+2].split("_")[1]}}》</span></p><div class="money"><p class="bookp">价格：<strong>{{buyinfo[5]}}阅点</strong><span>原价{{parseInt(buyinfo[1])*100}}阅点</span></p>' + '<p class="account"><span>余额：</span><strong>{{money}}</strong>阅点+<strong>{{zmoney}}</strong>阅券</p></div><div class="submit" @click.prevent="submit" v-if="isEnough">购买</div><a href="uniteqqreader://nativepage/coin/recharge" class="submit" v-else>余额不足，充值购买</a><i class="close" @click.prevent="hidemask"></i></div></div>',
				computed: {
					isEnough() {
