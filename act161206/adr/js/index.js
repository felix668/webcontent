	...
	el:"body",
 	data:{
		...
		iNow:param("picking")?8:0,
 		...
 		seeall:false, 		
 		over:false,
 		user:{isLogin:false,hasFree:false,shelf:false,uid:"",lt:""},
 		reload:false,
 		myScroll:null,
 		myScroll2:null
 	},
 	...
 	methods:{
 		init:function(){
			...
			//$('#box').css({"-webkit-transform":"translate3d(0,0,0) rotateX(-12deg) rotateY("+(-45-180*self.iNow)+"deg)","transform":"translate3d(0,0,0) rotateX(-12deg) rotateY("+(-45-180*self.iNow)+"deg)"}); 
		...
		sharepage:function(){
			if(!this.user.isLogin){
				setParam('picking',3);
				Local.login("http://iyuedu.qq.com/event/act161206/com/index.html?picking=3");
			 	return;
			}	
			var shareObj = {
				url :"https://yuedu.reader.qq.com/event/act161206/com/index.html?uid="+this.user.uid+"&lt="+this.user.lt,
				// url :prefix()+"act161206/com/index.html?uid="+this.user.uid+"&lt="+this.user.lt,
				cover :"http://iyuedu.qq.com/event/act161206/com/images/cover2.jpg",
				title: "偷偷告诉你",
