<template>
	<div class="lottery-box">
		<h2>峰峰发糖了</h2>
		<p class="tip">7月5日至7月11日，每日抽奖一次</p>
		<div class="gift-bg">
		...
		<div :class="['prizemask',{'showAlert':showAlert}]" v-if="showAlert">
			<div class="fail" v-if="noGift">
				<img class="p3" src="src/images/l3.png"></img>
				<p>今日抽奖机会已用完</p>	
				<div class="btn" @click="hidemask">–<strong>我知道了</strong>–</div>
			</div>
			<div class="succ" v-else>
				<img :class="'l'+result.prizeId" :src="result.pic"></img>
...
.prizemask{width: 100%;height: 100%;position: fixed;top:0;left:0;background: rgba(0,0,0,.6);z-index: 1000;@extend %centerflex;font-size: .32rem;
	...
	.fail{
		p{text-align: center;}
		img{width:.73rem;margin:.84rem auto .44rem}		
	}
	.btn{
		width: 3.92rem;
		line-height: .94rem;
	...
	.succ{
		.l0{width: 1.17rem;margin:.6rem auto;}
		.l1{width: 1.47rem;margin:.6rem auto;}
		.l2,.l4{width: 1.47rem;margin:.66rem auto .62rem;}
		.l3{width: .73rem;margin:.84rem auto .44rem;}
		.l5{width: 2.05rem;margin:.84rem auto .44rem;}
		.close{
...
<script>
	export default{
		...
		data(){
			return {
				islogin:this.mdata.islogin,
				audioTimeId: null,
				showAlert:false,//是否显示弹窗
				rewards:this.mdata.rewards,
		...
		methods: {
	 		startLottery(e){ 	
	 			...
	 			if (this.chance > 0) {	 		
	 				this.canclick=false;	
	 				//let data={code:1,prizeId:1};			
 					Local.reqaObj(`${common.server()}pkg170607/pick`, data=>{
	 		...
	 		lotteryResult(prizeId){
	 			...
	 			this.result=this.rewards[prizeId];
				//箭头转动
	 		...
	 		myprize(){
	 			...
	 			Local.openInside(`${common.front()}act170607/myprize.html`);
	 		}
	 	},
	 	computed:{
	 		version(){
