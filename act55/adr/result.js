...
function init() {
	...
	Index.INIT_URL = server() + 'pkg5/cardsLeft';
	Index.PICK_URL = server() + 'pkg5/pick';
	Index.LOTTERY_URL = server() + 'pkg5/doLottery';
...
Index.showReward = function(prizeId, prizeName) {
	if (prizeId > 0) {
		$("#mask6 .gPic").attr("src", "images/geted" + dictS[prizeId] + ".png");
		$(".gInfo").html(prizeName);
		$(".gname").html("获得" + prizeName);
		if (prizeId == 5) {
			$(".newtip").show()
		}
		ss("mask6", 'display', '-webkit-box');
	} else {
		ss("mask7", 'display', '-webkit-box');
	}
}
...
function mock(times, offset) {
	...
	d.setSeconds(0);
	d.setMilliseconds(0);
	var re = parseInt((new Date() - d) / (100 * times)) + offset;
	return re;
}