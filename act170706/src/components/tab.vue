<template>
	<ul class="tab" :class="{'fixtab':tab.fix}" id="tab" ref="tab">  
		<!-- <li style="color:red">{{tab.pos}}</li> -->
		<li :class="{'active':tab.pos==0}" :style="'background-image:url('+styleobj.tab+')'"></li>
		<li :class="{'active':tab.pos==1}" :style="'background-image:url('+styleobj.tab+')'"></li><li :class="{'active':tab.pos==2}" :style="'background-image:url('+styleobj.tab+')'"></li>
	</ul>
</template>	
	...
	export default {
		...
		methods:{
			...
			fixscroll:function(){
				...
				var t0=parseInt(document.querySelector("#column0").offsetTop-fixHeight);
				...
				for(var i=0;i<tablists.length;i++){
					(function(index){
						tablists[i].onclick=function() {
							...
							if(self.flog){
								Local.forceLog(common.param("act_f"),"TJStab");
							}
						}
					})(i)
				}			
