<template>
	<div class="plist" id="column2"> 
		...
		<div class="package" v-for="(item,index) in choosePackage" :style="`backgroundImage:url(${styleobj.columnbg2})`">
			<div class="pname">
				<h3>{{item.bagName}}</h3>
			</div>
			<img class="hot" :src="styleobj.hot" v-if="Math.max.apply([],pknum)==pknum[index]">
			<div class="pktag" :style="`backgroundImage:url(${styleobj.tag})`">{{item.discountBagMoney}}元购</div>
			<ul class="booklist">
		...
		methods:{
			...
			choose(n,index,event){			
				...
				if(this.choiceinfo[n].indexOf(index)>-1){
					...
					event.currentTarget.style.backgroundColor=this.styleobj.color1;
				}else if(this.choiceinfo[n].length<3){
					this.choiceinfo[n].push(index);
					event.currentTarget.style.backgroundColor=this.styleobj.color2;				
				}
			},
