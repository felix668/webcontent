...
ZBook.feedbackok=function (){ // 提交反馈到后台
	...
	Login.postaForm(server()+'/5/user2/feedback'+append,"content="+content);
};
...
ZBook.goQQBook=function(g_ut){
	if(g_ut==1){
		go('http://ebook.3g.qq.com/index?v=false&g_ut=1&sid='+lsg('sid'));
	}else if(g_ut==2){
		go('http://ebook.3g.qq.com/index?v=false&g_ut=2&sid='+lsg('sid'));
	}else if(g_ut=3){
		go('http://info.3g.qq.com/g/s?aid=index&g_ut=3&sid='+lsg('sid'));
	}
};
/** --- vip及收费扣费处理 小心点---- */
//跳转到包月开通
ZBook.goVipStep1=function(callback,b_f){
	Login.checkAndInvoke(function() {
		var bf=b_f;
		if(nl(bf)){
			bf = lsg("b_f");
		}
		var url=server()+'/5/vip/kaitong?sid='+lsg("sid")+'&b_f='+bf+'&g_f='+lsg("g_f")+'&indexPage='+BaseIndex.page();
		go(url);
	},callback);
};
//跳转到包月续费
ZBook.goPreFee=function(b_f){
	Login.checkAndInvoke(function() {
		...
		var url= 'http://ubook.3g.qq.com' +'/5/vip/xufei?sid='+lsg("sid")+'&b_f=' +bf+'&g_f='+lsg("g_f")+'&indexPage='+BaseIndex.page();		
		go(url);
	});
};
//跳转到充值中心
ZBook.chongzhi=function(b_f){ // b_f：不空用参数b_f，否则去本地b_f
	Login.checkAndInvoke(function() {
		...
		var url=server()+'lottery/chongzhiCP?sid='+lsg("sid") + '&b_f='+bf+'&g_f='+lsg("g_f")+'&indexPage='+BaseIndex.page()+'&currUrl='+back;		
		go(url);
	});
};
...
ZBook.buyFull=function(bid,cid){  // cid 进入阅读定位章节用
	...
	ZBook.buyFull_URL= server()+'/5/buy/buyFull?bid={0}';
	Login.reqaObj(format(ZBook.buyFull_URL,bid),function(dataObj){
		if(nl(dataObj)) {
			...
			if(dataObj.code==0){                  // 按本购买成功
				...
				indexStr=ZBook.buyResult_2('该书籍不能购买','返回上一页','javascript:ZBook.back();');
				...
				indexStr=ZBook.buyResult_2('包月用户不能购买包月书','返回上一页','javascript:ZBook.back();');
				...
				indexStr=ZBook.buyResult_2('书籍不存在，请阅读其他书籍','返回上一页','javascript:ZBook.back();');
			}else if(dataObj.code=8001){     // 章节不存在
				indexStr=ZBook.buyResult_2('章节不存在，请阅读其他书籍','返回上一页','javascript:ZBook.back();');
			}else if(dataObj.code==10000){   // 未登录
				Login.onLogin();
				return ;
			}else{              // 购买失败，请稍后重试
				indexStr=ZBook.buyResult_2('购买失败，请刷新页面后重试','返回上一页','javascript:ZBook.back();');
			}
			ZBook.openBuyTips(indexStr);
		}
	});
...
ZBook.buyChapter=function(bid,cid){
	...
	ZBook.buyChapter_URL= server()+'/5/buy/buyChapter?bid={0}&cid={1}';
...
ZBook.goRead=function(notice){
	if(notice==true && ZBook.checkDefaultNotice){
		...
		ZBook.cancelNotice_URL = server()+'/5/buy/cancelNotice';
