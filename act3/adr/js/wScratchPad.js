...
(function($)
{
	$.fn.wScratchPad = function(callback, option, settings)
	{
		...
		return this.each(function()
		{
			...
			var sp = new ScratchPad($settings, elem, callback);
	...
	{
		width		: 273,						// set width - best to match image width
		height		: 96,				// set height - best to match image height
		image		: 'images/not.png',		// set image path
		image2		: 'images/sbg.png',			// set overlay image path - if set color is not used
	...
	function ScratchPad(settings, elem, callback)
	{
		this.sp = null;
	...
	ScratchPad.prototype = 
	{
		generate: function()
		{
			...
			this.sp =
			$('<div></div>')
			.css({position: 'relative','-webkit-background-size':'100% auto','background-position':'center','background-repeat':'no-repeat'})
			.append(
				$(this.canvas)
		...
		scratchMove: function(e, $this)
		{
			...
			move_x=e.pageX;
			move_y=e.pageY;
			coo_X=(move_x-down_x)*(move_x-down_x);
			coo_Y=(move_y-down_y)*(move_y-down_y);
			coo_Len+=Math.sqrt(coo_X+coo_Y);
		},
