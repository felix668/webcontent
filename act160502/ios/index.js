...
function initp(login,expire,picked) {
	...
	var shareObj = {
		url : front()
				...
				+ (Index.u && bookarry.length ? "&bks=" + bookarry.join(",")
						: ""),
		cover : front()+"act160502/ios/images/share.jpg",
	 ...
	 * 设置titlebar上面的按钮
	 */
	Local.syncShareInfo(shareObj.title , shareObj.desc , shareObj.cover , shareObj.url);
	//点击选书
...
function addshelf(login) {
	if (login) {
		tips("确认加入书架？", "确认后，<br>选择免费读的书就不可更改了哦<br>(游客用户免费后需手动加书架)", "确认", "addbtn");
	} else {
		console.log("请登录");
		Local.login(null,'d');
	}
...
$("#addbtn").live("click", function() {
	Local.showToast("加入书架中,请稍后刷新书架查看~");
...
Index.toShare = function() {
	...
	Local.shareEventListen(shareObj.title, shareObj.desc, shareObj.cover,
			shareObj.url);
	Local.shareControl();
}