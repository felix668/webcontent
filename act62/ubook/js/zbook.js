...
ZBook.feedbackok=function (){ // 提交反馈到后台
	...
	if(nl(fbtime) || cint(fbtime)+ZBook.EIGHT_HOUR_MILLSSECONDS<time()){
		...
		Login.postaForm(server()+'/7/user/feedback'+append,"content="+content);
...
ZBook.goVipStep1=function(b_f){
	Login.checkAndInvoke(function() {
		...
		var url=server()+'/7/vip/kaitong?b_f='+bf+'&g_f='+lsg("g_f")+'&indexPage='+BaseIndex.page()+"&tt="+ttag();
		go(url);
	});
};
//跳转到包月续费
ZBook.goPreFee=function(b_f){
	Login.checkAndInvoke(function() {
		...
		var url=server()+'/7/vip/xufei?b_f=' +bf+'&g_f='+lsg("g_f")+'&indexPage='+BaseIndex.page()+"&tt="+ttag();		
		go(url);
	});
};
//跳转到充值中心
ZBook.chongzhi=function(b_f){ // b_f：不空用参数b_f，否则去本地b_f
	Login.checkAndInvoke(function() {
		...
		var url=server()+'/7/vip/chongzhi?b_f='+bf+'&g_f='+lsg("g_f")+'&indexPage='+BaseIndex.page()+'&currUrl='+back+"&tt="+ttag();		
		go(url);
	});
};
...
ZBook.buyFull=function(bid,cid){  // cid 进入阅读定位章节用
	...
	ZBook.buyFull_URL= server()+'/7/buy/buyFull?bid={0}';
...
ZBook.buyChapter=function(bid,cid){
	...
	ZBook.buyChapter_URL= server()+'/7/buy/buyChapter?bid={0}&cid={1}';
...
ZBook.goRead=function(notice){
	if(notice==true && ZBook.checkDefaultNotice){
		...
		ZBook.cancelNotice_URL = server()+'/7/buy/cancelNotice';
