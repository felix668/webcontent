...
//注册一件加入书架事件
Local.addToShelfBooks = function (bids) {
    var bookinfo;
    if (!isListenshelf) {
    ...
    for (var i = 0; i < booksinfo.length; i++) {
        bookinfo = booksinfo[i];
        document.dispatchEvent(addshelfevent);
    }
};

Local.reqaObj = function (url, callback) {
    // 需要添加必要的参数
    url += url.indexOf('?') != -1 ? '&' : '?';
...
//加入书架
Local.addToShelf = function (bookinfo) {
    C.callCocoa({
        "method": "addToShelf",
        "id": bookinfo.bid,
