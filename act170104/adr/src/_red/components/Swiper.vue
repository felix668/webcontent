	...
	<div class="Swiper"
		...
		<ul class="train" 
			...
			<li :class=" 'item '+(i===currentOne?'active':'') " 
				...
				<div class="content"
				style="background:url(img/red/card_2.png) no-repeat center;background-size:auto 100%;">
					<card0 v-if="i===0" :state="cardZero" :img="img"></card0>
					<card1 v-if="i===1" :state="cardOne" :img="img"></card1>
					<card2 v-if="i===2" :state="cardTwo" :img="img"></card2>
					<card3 v-if="i===3" :state="cardThree" :img="img"></card3>
				</div>
			</li>
...
.Swiper__ {
	...
	.Swiper {
		...
		width: 6.4rem; height: 8.28rem;
		margin: auto; margin-bottom: 0.38rem;
		//background: grey;
		//overflow: hidden;
		.train {
			width: 2000%;
