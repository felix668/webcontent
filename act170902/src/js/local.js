...
let Local = {}
if(window.pt=="adr"){
    Local = (function( JsBridge,common){
        ...
        //点击分享图片
        Local.shareStyleImage = function(bgimage,textimage,textObj){
            if(bgimage && textimage)
                try{
                    JSSns.shareStyleImage(bgimage,textimage,JSON.stringify(textObj));
                }catch (e){
        ...
        //漫画加书架
        Local.addComicToShelf = function(book, needToast) {
            // var info = obj(bookInfo);
            // info.resType = 3; // 1 文本，2听书，3漫画
            JSAddToShelf.addComic(book, needToast);
        };
        //去书籍详情页
        ...
        //跳转到相应金额的充值页
        Local.doCharge = function (count,from) {
          if (!from) {
            var from = "act"
          }
    ...
    Local = (function(C,common){
        ...
        Local.shareStyleImage = function(bgimage,textimage,textObj){
            var tmp = JSON.stringify(JSON.stringify(textObj))
            var result = tmp.slice(1,tmp.length-1)
            try{
                C.callCocoa({
                    "method": "shareStyleImage",
                    "bgimage": bgimage,
                    "textimage": textimage,
                    "textObj": result
                });
