...
var Local = (function( JsBridge,common ){
    ...
    Local.resApis = function () {
        JsBridge.restoreApis({
                'JSContent': ['setStatInfo', 'openExternal', 'openDetail', 'closeDialog', 'openMonthly', 'setWebTitlebarIcon', 'openTopicComment', 'openTopicDiscuss', 'setIsBackToPage','getUsedSkinId','obtainGift'],
    ...
    Local.addToShelf = function(book, needtoast) {
        if (needtoast == undefined)
            needtoast = 1;
        var bookinfo = '{"id":"' + book.bid + '",' + '"title":"' + book.title
            + '",' + '"author":"' + book.author + '",' + '"isfinished": "'
            + book.finished + '", ' + '"type":"0",' + // 为保持调试正确保留，下一版本待删除
            '"offset":"0",' + '"coverurl":"' + book.cover + '",'
            + '"downloadurl":"' + "" + '",' + '"maxchapter":'
            + book.lastChapter + ',' + // 就是客户端称其为版本的地方，这里又改回了lastChapter
            '"drm":"' + "" + '", ' + '"format":"txt", ' + '"downloadable":"'
            + book.lastChapter + '",' + '"chapterid":"1",'
            + '"chaptertitle": "' + "" + '", ' + '"needtoast": ' + needtoast
            + ', ' + '"updatetime":""}';
        try {
            JSAddToShelf.add(bookinfo);
        } catch (e) {
        }
    };
    /**
    ...
    Local.$toBook = function(bid){
         console.log('[local.js] 跳转到书籍详情页...')
        JSContent.openDetail( JSON.stringify({
            pagecode: 1001,
            bid: bid
        }) );
    }
    Local.$toRead = function(bid){
        console.log('[local.js] 跳转到书籍阅读页...')
        JSContent.openDetail( JSON.stringify({
            pagecode: 1015,
            bid: bid
        }) );
    }
    Local.$toBaoyueZone = function(){
        console.log('[local.js] 跳转到包月专区...')
        JSContent.openDetail( JSON.stringify({
            pagecode: 1020
        }) );
    }
    Local.$toJingxuan = function(){
        console.log('[local.js] 跳转到精选页...')
        JSContent.openDetail( JSON.stringify({
            pagecode: 1013
        }) );
    }
    ...
    Local.forceLogTemp = function( pageNumber,act_b,count ) {
        var act_by = act_b || pageName;
        Local.reqaObj( server() + 'topic/oldcz/sum?pagenumber='+pageNumber+'&act_b='+act_by+'&count='+count,
            function(data) {
                //alert(JSON.stringify(data));
            }, [], function() {
            }, 1);
    ...
    Local.$share = function(url,img_url,title,intro){
        Local.shareTopic(
            url,
            img_url,
            title,
