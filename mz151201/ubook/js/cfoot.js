...
if (window.location.hostname == _CONF.FORMAL_HOST) {
	_SERVER = _CONF.FORMAL_SERVER; // 正式环境
} else if (window.location.hostname == _CONF.TEST_HOST) {
	test=true;
	_SERVER = _CONF.TEST_SERVER; // 测试环境
} else if (window.location.hostname == _CONF.LOCAL_HOST) {
	test=true;
	_SERVER = _CONF.TEST_SERVER; // 测试环境
} else
	_SERVER = _CONF.FORMAL_SERVER;// 正式环境
...
function ga(eid, attribute) {
	if (typeof eid == 'string')
		return id(eid)[attribute];
	else
		return eid[attribute];
}
// 设置对象属性
function sa(eid, attribute, value) {
	if (typeof eid == 'string')
		id(eid)[attribute] = value;
	else
		eid[attribute] = value;
}
// 获取CSS属性
function gs(eid, property) {
	if (typeof eid == 'string')
		return getComputedStyle(id(eid), null).getPropertyValue(property);
	else
		return getComputedStyle(eid, null).getPropertyValue(property);
}
// 设置CSS属性
function ss(eid, property, value) {
	if (typeof eid == 'string')
		id(eid).style[property] = value;
	else
		eid.style[property] = value;
}
// 获取整型CSS属性
...
function sstxt(eid, classtxt) {
	if (typeof eid == 'string')
		id(eid).style.cssText = classtxt;
	else
		eid.style.cssText = classtxt;
}
// 设置A标签href的url
function ah(eid, url) {
	sa(eid, 'href', url);
...
function ud(eid, name) {
	if (typeof eid == 'string')
		return id(eid).getAttribute('data-' + name);
	else
		return eid.getAttribute('data-' + name);
}
// 对象为空判断
function nl(obj) {
...
function center(eid, parentLeft, parentTop, parentWidth, parentHeight) {
	mxyt(eid, parentLeft + parseInt((parentWidth - wf(eid)) / 2), parentTop
					+ parseInt((parentHeight - hf(eid)) / 2));
}
// 宽度改变
...
function bind(eid, event, callback) {
	if (typeof eid == 'string')
		id(eid).addEventListener(event, callback, false);
	else
		eid.addEventListener(event, callback, false);
}
// 获取触摸点横坐标
function ex(event) {
