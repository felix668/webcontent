...
var common = (function(){
  var _CONF = {
    ...
    TEST_SERVER: 'https://ptactivity.reader.qq.com/activity/', // 测试环境后台
    ...
    FORMAL_SERVER: 'https://activity.reader.qq.com/activity/', // 正式环境后台
...
if(window.pt=="adr"){
    var Local = (function( JsBridge,common){
        ...
        Local.forceLog = function(act_f, act_b, actid, moduleid) {
            var act_from = act_f || -110;
            var act_by = act_b || pageName;
            Local.reqaObj( server() + "sum?act_f=" + act_from + "&act_b=" + act_by + "&actid=" + actid + "&moduleid=" + moduleid,
                function(data) {
        ...
        //单本书加入书架  needtoast 0表示不弹出toast 1表示弹出toast
        ...
        Local.addToShelfBooks = function (books) {
            for (var i = 0; i < books.length; i++) {
                ...
                //if (i == books.length - 1) {
                   // Local.addToShelf(bookinfo, 1); //安卓加书架的提示貌似没有用
                //} else {
