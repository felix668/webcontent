 ...
 */
Local.reqaObj = function (url, callback, callbackSections, failCallback, forceReload) {
    ...
    if (param("tf") && param("tf") == 1) {
        ...
        reqa(url, function (txt) {
            doPrintSection(txt, failCallback, callbackSections, callback, 'false');
        });
    } else {
        //  调用客户端的代码就是用下面的函数
        Local.reqByClient(url, function (txt) {
            doPrintSection(txt, failCallback, callbackSections, callback, 'true');
        }, forceReload);
    }
};
 ...
 * @param errcallback
 */
Local.reqaObjByWeb = function (url, callback, errcallback) {
    url = Local.getTransparentParam(url);
    // pc上浏览测试
    reqa(url, function (txt) {
        if (nl(txt)) {
...
//跳转到书友圈
Local.goCommenyArea = function (bid) {
    var pagetitle = "书友圈";
    var detailobj = {};
...
//跳转到书友圈
Local.goPostcomment = function (bid) {
    var pagetitle = "发表书评";
    var detailobj = {};
...
//跳转到书友圈
Local.goCommentDetail = function (bid, commentid) {
    var pagetitle = "书评详情";
    var detailobj = {};
