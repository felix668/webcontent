...
function server() {
	if (window.location.hostname == _CONF.FORMAL_HOST){
		...
		return "https://event.reader.qq.com/";
	}else{
...
function openurl(url) {
	if (window.location.hostname == _CONF.FORMAL_HOST){
		if(env.pt=="adr"){
			return "http://iyuedu.qq.com/event/" + url + "/adr/index.html";
		}else if(env.pt="ios"){
			return _CONF.FORMAL_FRONT + url + "/ios/index.html";;
		}
	}else{
		if(env.pt=="adr"){
			return "http://solomotest4.3g.qq.com/book_res/event/" + url + "/adr/index.html";
		}else if(env.pt=="ios"){
			return _CONF.TEST_FRONT + url + "/ios/index.html";;
		}		
	}
}
// 构造当前页面请求参数表
