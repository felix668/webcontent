	...
	el:"#root",
 	...
 	methods:{
 		...
 		readpinit:function(){
 			var self=this;
 			//获取阅读红包的数据
			Local.reqaObj("http://common.reader.qq.com/v6_3_7/nativepage/readTime/getWeekExchangeInfo", function(data) {
				if(data.code==0){
		...
		getRpack:function(){
			...
			var self=this;
			//领取阅读红包
			Local.reqaObj("http://adr.reader.qq.com/6_3_7/weekReadTime/presentBillingV2", function(data) {
				if(data.code==0){
		...
		share:function(){
			var shareObj = {
				url : common.sharefront()+"act170106/com/index.html",
				cover :common.sharefront()+"act170106/com/images/cover.jpg",
				title: "三重红包送不停",
			...
			for(var i=0;i<tablists.length;i++){
				(function(index){
					tablists[i].onclick=function() {
						...
						MtaH5.clickStat('tab'+index,{'plat':'adr'});
					}
				})(i)
			}			
		}
