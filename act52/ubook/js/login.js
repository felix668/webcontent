var Login={};
Login.goURL = 'http://ui.ptlogin2.qq.com/cgi-bin/login?pt_ttype=1&style=8&appid=1600000062&daid=308&hln_css='+encodeURIComponent('http://ubook.qq.com/5/img/ptlogo.png')+'&s_url=';
...
Login.login=function(){ 
	if(!Login.isLogin()) {
		//转到登录地址，登录成功刷新当前页面
		url=window.location.href.replace('sid=', 'dis=');
		...
		url+=url.indexOf('?')>0?'&ptlg=1':'?ptlg=1';
...
Login.buildURL=function(aurl) {
	...
	//对于sid的统一处理
	url+=url.indexOf('?')!=-1?'&':'?';
	url+=nl(lsg("sid"))?"sid=null":('sid='+lsg("sid")); //预先取一下sid，附在后边
...
//仅当有必要时，才保存外界输入的sid：
//链接地址携带sid参数以传递用户身份的方式，是落后的，因而对链接地址sid参数的处理不应被合并入buildURL，而是仅在个别入口页面做特殊处理即可。
//FIXED BY p_yydeng 因为活动链接放在触屏环境下，用户信息以前端传入的sid为准
Login.saveSidWhenNecessary=function() {
//	if(nl(lsg('sid')) || lsg('isLogin')!='1') {
		var psid=param('sid');
		if(!nl(psid)) {
			lss('sid',psid);
			var ppt=param('ptlg');
			if(ppt=='1') lss('isLogin','1');
		}
//	}
};
...
BaseIndex.qdinit=function() {
	if(BaseIndex.day15<time()-cint(nlg(lsg('qdclose'),'0'))){
		scls('qd1','pad60');
		dv('qd2');
	}
};
...
BaseIndex.req=function(num) {
	return server()+ '/5/'+BaseIndex.urls[BaseIndex.id]+'index'+num;
};
