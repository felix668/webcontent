 ...
 * @param callbackSections
 */
Local.postObj = function (url, params, callback, callbackSections, faildCallback, forceReload) {
    url = Local.getTransparentParam(url);
    // 默认不强制刷新数据
    if (!forceReload) {
        forceReload = 0;
    ...
    if (param("tf") && param("tf") == 1) {
        //  pc上浏览测试
    	postdata(url, params, function (txt) {
    		doPrintSection(txt, faildCallback, callbackSections, callback, 'true');
        });
    } else {
        //  调用客户端的代码就是用下面的函数
        Local.postByClient(url, function (txt) {
            doPrintSection(txt, faildCallback, callbackSections, callback, 'true');
        }, forceReload, params);
    }
};
