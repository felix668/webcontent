...
var _CONF={
    ...
    DEVELOP_SERVER:	'http://test3.wapmusic.qq.com/qbsr/', 	//开发环境后台服务地址

	TEST_HOST:		'solomotest4.3g.qq.com',				//测试环境访问地址
	TEST_LOCAL_HOST:'http://solomotest4.3g.qq.com/book_res/event/act4/', //测试环境访问地址
	TEST_SERVER:	'http://test3.wapmusic.qq.com/qbsr/',	//测试环境后台服务地址
	
	FORMAL_HOST:	'ireader.qq.com',					//正式环境访问地址
	FORMAL_LOCAL_HOST:'http://ireader.qq.com/event/act4/', //正式环境页面地址
	FORMAL_SERVER:	'http://event.book.qq.com/qbsr/'		//正式环境后台服务地址
};
//============ 全局回调 ============
//全局入口 ginit
if(window.ginit) document.addEventListener('DOMContentLoaded',ginit,false);
//if(window.ginit) window.addEventListener('pageshow',ginit,false); //2014.10.23龙海建和刘人方建议使用这个事件，解决浏览器回退不能感知问题，结果发现定时任务错乱、对话框文字丢失等。
...
//配置前台页面地址
var _FRONT=_CONF.TEST_LOCAL_HOST; //开发环境
if(window.location.hostname==_CONF.FORMAL_HOST) _FRONT=_CONF.FORMAL_LOCAL_HOST; //正式环境
else if(window.location.hostname==_CONF.TEST_HOST) _FRONT=_CONF.TEST_LOCAL_HOST; //测试环境
//获取后台服务地址
function server() { 
	return _SERVER;
}
...
//异步请求2，回调函数的入参是响应文本。
function reqa2(url,callback) {
	url+=url.indexOf('?')!=-1?('&tt='+ttag()):('?tt='+ttag());
	var areq=new XMLHttpRequest();
	areq.withCredentials = true; //跨域传Cookie，后台也要加相关设置
	areq.onload=function() {
		callback(areq.responseText);
	};
	areq.onerror=function() {
...
//异步发送表单数据2，回调函数的入参是响应文本
function posta2(url,value,callback) {
	url+=url.indexOf('?')!=-1?('&tt='+ttag()):('?tt='+ttag());
	var areq=new XMLHttpRequest();
	areq.withCredentials = true; //跨域传Cookie，后台也要加相关设置
	areq.onload=function() {
