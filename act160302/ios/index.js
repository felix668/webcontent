...
Index.logined = function(data) {
	...
	if(data.canPick){
		...
		if (data.p1) {
			...
			$("#btn50").text("再消耗" + parseInt(50 - bookcoin) + "阅点可领取");
		} else {
			$("#btn50").text("立即领取");
		}
		if (data.p2) {
			...
			Index.disables("#btn500");
			$("#btn500").text("再消耗" + parseInt(500 - bookcoin) + "阅点可领取");
		} else {
			$("#btn500").text("立即领取");
		}
		if (data.p3) {
			...
			Index.disables("#btn5000");
			$("#btn5000").text("再消耗" + parseInt(5000 - bookcoin) + "阅点可领取");
		} else {
			$("#btn5000").text("立即领取");
		}
	}else{
...
Index.result = function(data,btn) {
	if (data.code == 1) {//领取成功
		...
		if(btn=="btn500"){$("#no_card .other").text("10阅券");}
		else if(btn=="btn5000"){$("#no_card .other").text("188阅券");}
	}else{
