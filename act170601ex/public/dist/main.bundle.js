  ...
  INIT_MAIN: function INIT_MAIN(_ref2, action) {
    ...
    if (state.meta.share) {
      ...
      if (state.dev) {
        ...
        var url = common.serverRoot() + 'api/pkg170601ex/init?plat=' + plat;
        var cmap = cks();
          var uin = null;
        if(cmap !=null && cmap.uin !== undefined){
          uin = cmap.uin;
          var skey = cmap.skey;
          url =  url + "&uin=" + uin + "&skey=" + skey;
        }

        //var url = common.serverRoot() + 'api/pkg170601ex/init?plat=1';
        ...
        Local.reqaObj(url, function (data) {
          ...
          if (data.code === -4) {
            ...
            if (data.verok === false) {
              ...
              data.isLogin = Login.isLogin(); // 由前端登录态决定
              if (data.isLogin) {
                state.user.loggedIn = true;
                // if (data.verok === false) {
                //   state.data.latest = false;
                ...
                if (data.picked > 0) { //已经领取过书券了
                  console.log("You have picked tickets: " + data.picked);
                  state.data.takenBefore = true;
                  state.data.bills_taken = data.picked;
                } else if (data.noneleft === true) {
                  state.data.out = true; // 书券发完了
    ...
    if (state.meta.share) {
      ...
      if (state.dev) {
        ...
        Local.reqaObj(url, function (data) {
          ...
          } else if (data.code === -11) { // 奖品领取完
            state.data.canScrape = true;
            state.data.bills_taken = -1;
          } else if (data.code === -12) { // 刮奖失败
            dispatch({
              type: 'SHOW_MASK',
              case: 'failed'
            });
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    on: {
        "click": function($event) {
            var productid = "com.tencent.qqreaderiosv56.vip.3month";
            nss("productid", productid);
            go("./myaccount.html" + "?productid=" + productid);
            /*
             _vm.$store.dispatch({
             type: 'TO_BUY',
             month: 3
    ...
    on: {
        "click": function($event) {
            var productid = "com.tencent.qqreaderiosv56.vip.6month";
            nss("productid", productid);
            go("myaccount.html" + "?productid=" + productid);
            /*
             _vm.$store.dispatch({
    ...
    on: {
        "click": function($event) {
            var productid = "com.tencent.qqreaderiosv56.vip.12month";
            nss("productid", productid);
            go("myaccount.html" + "?productid=" + productid);
            /*
             _vm.$store.dispatch({
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  ...
  return _c('p', [_c('span', {
    staticClass: "no_"
  }, [_vm._v("1.")]), _vm._v("活动时间：2017年6月1日-6月11日。")])
}]}
