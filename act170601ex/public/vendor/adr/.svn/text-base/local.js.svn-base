...
var Local = (function( JsBridge,common ){
    ...
    function reqa(url, callback, errcallback) {
        ...
        console.log("Local:" + url + ";" + callback);
    ...
    Local.cacheImage = function(url){
        if(!!url){
            try{
                JSSns.cacheImage(url);
            }catch (e) {
                Local.showToast("cacheImage fail");
            }
        }
    }
    Local.shareStyleImage = function(bgimage,textimage){
        if(bgimage && textimage)
            try{
                JSSns.shareStyleImage(bgimage,textimage);
            }catch (e){
                Local.showToast("shareStyleImage fail");
            }
    }
