//【本函数库唯一需要手工配置的地方】
var _CONF={
    LOCAL_HOST:		'http://solomotest4.3g.qq.com/book_res/event/act170601ex/public/', //本地开发访问地址
    DEVELOP_SERVER:	'http://test3.wapmusic.qq.com/ubook/', 	//开发环境后台服务地址

	TEST_HOST:		'solomotest4.3g.qq.com',				//测试环境访问地址
	TEST_LOCAL_HOST:'http://solomotest4.3g.qq.com/book_res/event/act170601ex/public/', //测试环境访问地址
	TEST_SERVER:	'http://test3.wapmusic.qq.com/ubook/',	//测试环境后台服务地址
	
	FORMAL_HOST:	'iyuedu.qq.com',					//正式环境访问地址
	FORMAL_LOCAL_HOST:'http://iyuedu.qq.com/event/act170601ex/public/', //正式环境页面地址
	...
	if(i>0) {
		...
		if(str1.length>0) {
			...
			for(var i=0;i<arr.length;i++) {
				var kv=arr[i].split('=');
				if(kv.length>=2) _PARAMS[kv[0]]=kv[1];
			}
		}
	}
})();
