...
Index.onloads=function(){
    ...
    setTimeout(function(){
        ...
        //$("#recomlist").css({"width":lenH*winLi,"-webkit-transform":"translate("+(-winLi)+"px,0px)","overflow":"hidden"});
        $("#recomlist").css({"width":lenH*winLi,"left":(-winLi)+"px","overflow":"hidden"});
...
Index.nograde = function() {
    ...
    $('#updateBtn').on('touchend', function () {
        ...
        $("#updateBtn").attr("href","http://misc.wcd.qq.com/app?packageName=com.qq.reader&channelId=10000382");
    ...
    $(elem).on("touchmove",function(ev){
        ...
        $(elem).on("touchend",function(ev){
            ...
            var offsetX=x2-touchStartX;
                ...
                if(offsetX<0){//向左滑
                    iNow+=1;
                    if(iNow==lenH){iNow=lenH-1;}
                }
                if(offsetX>0){//向右滑
                    iNow-=1;
                    if(iNow<0){iNow=0;}
                }
                //$(elem).css({"transition":"1s","-webkit-transform":"translate3d("+(-iNow*winLi)+"px,0px,0px)"});
                $(elem).css({"transition":"0.6s","left":(-iNow*winLi)+"px"});
        });
};
