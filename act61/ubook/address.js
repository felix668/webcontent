...
Address.addAdress = function() {
	Login.reqaObj(Address.URL + "?act_b="+$(".bgqq").val()+"--"+$(".bgtel").val()+"--"+$("#adress").val()+"&act_f=99999", function(data) {
		console.log(data);
		if(data.isLogin){
			$("#notice").show();
		}else{
			$("#notice h5").text("请重新登陆~");
			$("#notice").show();
		}
	}, [], function() {
...
Address.forceLog = function(act_f, act_b) {
	...
	Login.reqaObj(Address.URL + "?act_f=" + act_from + "&act_b=" + act_by,
			function() {
			});
};