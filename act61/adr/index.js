...
Index.fillPage = function() {
	Local.reqaObj(Index.INIT_URL + "&act_f=" + param("act_f"), function(data) {
		if (data.isLogin) {
			$("#cz").attr("href", "javascript:Index.chongzhi(true)");
			$(".openbtn").attr("href", "javascript:Index.openVIP(true)");
		} else {
			$("#cz").attr("href", "javascript:Index.chongzhi(false)");
			$(".openbtn").attr("href", "javascript:Index.openVIP(false)");
		}
...
Index.chongzhi = function(isLogin) {
	Local.doCharge('act', isLogin);
}
...
Index.dojiayou = function(type) {
	...
	Local.reqaObj(Index.REWARD_URL + "?act_f=" + (param("act_f") || -110)
