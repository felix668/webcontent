var vm=new Vue({
	el:"#root",
 	data:{
 		...
 		readpack:{"canExchangeBilling":0,"curexchangedBilling":0,"exchangeBilling":0,"needReadTime":0,"weekreadtime":0,"hasFinishExchange":false},
 		loading:true,
 		isLogin:false,
 		over:false,
 		tab:1,//是否开始 1未开始 2开始
 	...
 	methods:{
 		...
 		readpinit:function(){
 			var self=this;
 			//获取阅读红包的数据
			Local.reqaObj("http://adr.reader.qq.com/6_3_6/getWeekReadTime", function(data) {
				self.readpack.icon=data.icon || "images/default.png";
		...
		getDpack:function(){
			...
			MtaH5.clickStat('btn',{'plat':'oldadr','name':'btn0'});
		},
		getRpack:function(){
			if(!this.isLogin){
				this.gologin(3);
			...
			var self=this;
			//领取阅读红包
			Local.reqaObj("http://adr.reader.qq.com/6_3_6/weekReadTime/presentBilling", function(data) {
				if(data.code==0){
			...
			MtaH5.clickStat('btn',{'plat':'oldadr','name':'btn1'});
		},
		hidemask:function(){
			this.mask=false;
			this.rule=false;
		...
		share:function(){
			var url=(function () {
				...
				return pre + '/act170106/com/index.html?act_f=' + common.param('act_f');
			})();
			var shareObj = {
				url : url,
				cover :"https://yuedu.reader.qq.com/event/act170106/com/images/cover.jpg",
				title: "三重红包送不停",
			...
			for(var i=0;i<tablists.length;i++){
				(function(index){
					tablists[i].onclick=function() {
						...
						MtaH5.clickStat('tab',{'plat':'oldadr','name':'tab'+index});
					}
				})(i)
			}	
		}
