			...
			<template v-else>
				...
				<ul class="plist" v-if="plist.length>0">
					<li v-for="item in plist">
						<div class="pic">
							<img :src="'src/images/'+prizes[item.first]+'.png'">
						</div>
						<p>{{item.second}}</p>
					</li>
				</ul>
	...
	export default {
		...
		created(){ 
	 	...
	 	methods:{
	 		...
	 		init(){
	 			// let data={isLogin:true,myPrize:[{first:0,second:"111"},{first:1,second:"111"},{first:2,second:"111"},{first:3,second:"111"},{first:4,second:"111"},{first:5,second:"111"}]};
				Local.reqaObj(`${common.server()}pkg170608/myPrize`, data=> { 
