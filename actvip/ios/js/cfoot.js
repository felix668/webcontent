...
function getTransparentParam(){
	...
	str = seturlparam(str, "act_f");//统计专用
	
	return str;
}

function server() {
	if (window.location.hostname == _CONF.FORMAL_HOST || window.location.hostname == _CONF.FORMAL_HOST_HTTPS)
		return location.protocol == "https:" ? _CONF.FORMAL_SERVER_HTTPS : _CONF.FORMAL_SERVER;
	else
		return location.protocol == "https:" ? _CONF.TEST_SERVER_HTTPS : _CONF.TEST_SERVER;
}
...
function genBookPic(bid, prefix){
	prefix = prefix || 's';
	if (bid) {
		return (location.protocol == "https:" ? 'https://static.reader.qq.com/cover/':'http://wfqqreader.3g.qq.com/cover/') + (bid % 1000) + "/" + bid + "/" + prefix + "_" + bid + ".jpg";
	}
	return (location.protocol == "https:" ? 'https://static.reader.qq.com/cover/':'http://wfqqreader.3g.qq.com/cover/') + prefix + "_default.jpg";
}
