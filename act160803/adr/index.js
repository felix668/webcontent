...
//Index.isLoaded(["834076","834076","833332","833331"]);
//未登录
...
//关闭弹窗
$(".knowbtn,.close").on("touchend",function(ev){
	ev.preventDefault();
	Index.maskOut();
});
//抽奖结果
Index.result=function(data){
	...
	if(data==4){//书券
		Index.know("书券已即时到账，15天有效<br>请尽快使用");		
	}else if(data==6){//包月
		Index.know("包月会员已开通，10万册包月书<br>可以在线免费看7天");
	}else{
		Index.contact();
	}
